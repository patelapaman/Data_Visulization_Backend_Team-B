...
def export_events_csv():
    """Export the filtered event result as a frontend-friendly CSV download."""
    try:
...
def analytics_overview_api():
    return jsonify({"status": "success", "data": overview_analytics(_filters())})
...
def threat_timeline_api():
    ...
    return jsonify(
        {"status": "success", "data": threat_timeline(_filters(), interval=interval)}
    )
...
def top_targeted_assets_api():
    ...
    return jsonify(
        {"status": "success", "data": top_targeted_assets(_filters(), limit=limit)}
    )
...
def heatmap_api():
    return jsonify({"status": "success", "data": heatmap_data(_filters())})


@addons_bp.get("/api/reports/threats.pdf")
@addons_bp.get("/api/reports/pdf")
def threat_report_pdf():
    """Generate a compact PDF report from the same live MongoDB analytics."""
    filters = _filters()
    ...
    line(f"Generated: {datetime.utcnow().isoformat(timespec='seconds')} UTC", 9, 20)
    line("Threat Analytics Summary", 13, 20)
    line(f"Severity: {', '.join(f'{item['label']}={item['count']}' for item in analytics['severity'])}")
    line(f"Event types: {', '.join(f'{item['label']}={item['count']}' for item in analytics['event_types'][:8])}")
    line("Top targeted assets", 12, 20)
    for item in assets:
        line(
            f"{item['asset']} | events={item['event_count']} | "
            f"critical={item['critical_events']} | avg risk={item['average_risk_score']}"
        )
    line("Threat timeline", 12, 20)
    for item in timeline[-20:]:
        line(
            f"{item['period']} | events={item['events']} | "
            f"high risk={item['high_risk_events']} | critical={item['critical_events']}"
        )
    document.save()
    output.seek(0)
    response = Response(output.getvalue(), mimetype="application/pdf")
    response.headers["Content-Disposition"] = "attachment; filename=threat-report.pdf"
    return response